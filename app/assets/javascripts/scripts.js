$(document).ready(function() {
  ...
  // Toggle forms
  $('.show-form').click(function() { $(this).next('.row').toggleClass('hidden'); });
  $('.cancel-form').click(function() { $(this).parent().parent().parent('.row').toggleClass('hidden'); });
  $('.send-now').click(function() { $(this).children('i').toggleClass('hidden'); });

  // Live update stylesheet
  ...
  // Reset style inputs
  $('.form-group').hover(function() { $(this).children('.reset-input').children('span').toggleClass('hidden'); });
  $('.reset-input').click(function() {
    input = $(this).prev().children().children('input');
    input.val(input.data('original')).change();
    input.next().children('span').css('background-color', input.data('original'));
  });
